...
async function uploadImage(page, filename) {
  await page.goto('/');
  await page.locator('input[type="file"][name="image"]').setInputFiles(path.join(FIXTURES, filename));
...
test.describe('Stage 5 — Image Detail, Download & Delete', () => {
  ...
  test('GET /image/:id/download sets Content-Disposition attachment header', async ({ page, request }) => {
    await uploadImage(page, 'red.jpg');
    const id = await getLastImageId();
